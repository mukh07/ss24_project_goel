...
def get_args():
    ...
    parser.add_argument('--parsing_data_path', type=str, required=True, help="path of Gait3d parsing data, must be the absolute path.")
...
def main():
    ...
    #all_parsing_files = sorted(glob(os.path.join(parsing_data_path, f"*/*/*/*{opt.dataset_pkl_ext_name}")))
    ...
    if len(all_heatmap_files) >= len(all_silouette_files):
        for heatmap_file in tqdm(all_heatmap_files):
            ...
            parsing_folder=os.path.join(parsing_data_path, *tmp_list[-4:-1])
            ...
            if not os.path.exists(parsing_folder):
                ...
                parsing_file = sorted(glob(os.path.join(parsing_folder, f"*{opt.dataset_pkl_ext_name}")))[0]
            ...
            os.system(f"ln -s {parsing_file} {output_file}/2_parsing.pkl")
    else:
