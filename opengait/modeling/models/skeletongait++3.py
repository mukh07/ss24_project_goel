...
class SkeletonGaitPP3(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.parsing_layer1 = copy.deepcopy(self.sil_layer1)
        ...
        elif(model_cfg['fusion']=="Cat"):
            print("Cat Fusion")
            self.fusion=CatFusion(32 * C)
        else:
    ...
    def inputs_pretreament(self, inputs):
        ...
        pose_sils_parsings = inputs[0]
        new_data_list = []
        for pose, sil, parsing in zip(pose_sils_parsings[0], pose_sils_parsings[1], pose_sils_parsings[2]):
            sil = sil[:, np.newaxis, ...] # [T, 1, H, W]
            parsing = parsing[:, np.newaxis, ...] # [T, 1, H, W]
            ...
            cat_data = np.concatenate([pose, sil, parsing], axis=1) # [T, 4, H, W]
    ...
    def forward(self, inputs):
        ...
        maps = pose[:, :2, ...]
        sils = pose[:, 2, ...].unsqueeze(1)
        parsings = pose[:, 3, ...].unsqueeze(1)
        ...
        parsing0 = self.parsing_layer0(parsings)
        parsing1 = self.parsing_layer1(parsing0)

        out1 = self.fusion(sil1, map1, parsing1)
...
class AttentionFusion(nn.Module): 
    def __init__(self, in_channels=64, squeeze_ratio=16):
        ...
        self.conv = SetBlockWrapper(
            nn.Sequential(
                conv1x1(in_channels * 3, hidden_dim), 
                nn.BatchNorm2d(hidden_dim), 
                nn.ReLU(inplace=True), 
                conv3x3(hidden_dim, hidden_dim), 
                nn.BatchNorm2d(hidden_dim), 
                nn.ReLU(inplace=True), 
                conv1x1(hidden_dim, in_channels * 3), 
            )
        )
    
    def forward(self, sil_feat, map_feat, parsing_feat): 
        '''
            sil_feat: [n, c, s, h, w]
            map_feat: [n, c, s, h, w]
            parsing_feat: [n, c, s, h, w]
        '''
        c = sil_feat.size(1)
        feats = torch.cat([sil_feat, map_feat, parsing_feat], dim=1)
        score = self.conv(feats) # [n, 3 * c, s, h, w]
        score = rearrange(score, 'n (d c) s h w -> n d c s h w', d=3)
        score = F.softmax(score, dim=1)
        retun = sil_feat * score[:, 0] + map_feat * score[:, 1] + parsing_feat * score[:, 2]
        return retun
...
class PlusFusion(nn.Module): 
    ...
    def forward(self, sil_feat, map_feat, parsing_feat): 
        '''
            ...
            parsing_feat: [n, c, s, h, w]
        '''
        return sil_feat + map_feat + parsing_feat
